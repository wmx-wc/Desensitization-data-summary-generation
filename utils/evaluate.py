...
from utils.utils import Smoother
...
def evaluate(model, loader, args, device):
    ...
    loader = tqdm(loader, total=num_steps)
    for i, batch in enumerate(loader):
        input_ids, masks, lm_labels = batch
        # Generate Summary
        summary_ids = model.generate(input_ids.to(device), attention_mask=masks.to(device), num_beams=args.num_beams,
                                     max_length=96, early_stopping=True, eos_token_id=1)
        summary_ids = summary_ids.tolist()
        print(lm_labels)
        for summary, label in zip(summary_ids, lm_labels):
            end = 2
            ...
            pred = ' '.join([str(x) for x in summary])
